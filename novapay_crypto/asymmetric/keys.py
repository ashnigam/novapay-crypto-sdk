...
from cryptography.hazmat.primitives.asymmetric.ec import ECDH, SECP256R1, SECP384R1, SECP521R1, EllipticCurvePrivateKey, EllipticCurvePublicKey
...
def generate_rsa_keypair(key_size: int=RSA_DEFAULT_KEY_SIZE) -> Tuple[RSAPrivateKey, RSAPublicKey]:
    ...
    """
    private_key = generate_private_key(public_exponent=65537, key_size=key_size, backend=default_backend())
...
def export_private_key_pem(private_key: RSAPrivateKey | EllipticCurvePrivateKey, password: bytes | None=None) -> bytes:
    ...
    return private_key.private_bytes(serialization.Encoding.PEM, serialization.PrivateFormat.PKCS8, encryption)
...
def public_key_to_jwk(public_key: RSAPublicKey | EllipticCurvePublicKey) -> dict:
    ...
    if isinstance(public_key, RSAPub):
        ...
        crv = {SECP256R1: 'P-256', SECP384R1: 'P-384', SECP521R1: 'P-521'}.get(type(curve), 'P-256')
...
def ecdh_shared_secret(private_key: EllipticCurvePrivateKey, peer_public_key: EllipticCurvePublicKey, info: bytes=b'novapay-session-key') -> bytes:
