...
from cryptography.x509 import Certificate, CertificateBuilder, DNSName, IPAddress, NameAttribute, NameOID, random_serial_number
from cryptography.x509.oid import ExtendedKeyUsageOID
logger = logging.getLogger(__name__)

def generate_self_signed_cert(common_name: str, san_domains: List[str] | None=None, san_ips: List[str] | None=None, key_size: int=2048, valid_days: int=365) -> tuple[RSAPrivateKey, Certificate]:
    ...
    subject = issuer = x509.Name([NameAttribute(NameOID.COUNTRY_NAME, 'US'), NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, 'California'), NameAttribute(NameOID.LOCALITY_NAME, 'San Francisco'), NameAttribute(NameOID.ORGANIZATION_NAME, 'NovaPay Development'), NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, 'Engineering'), NameAttribute(NameOID.COMMON_NAME, common_name)])
    now = datetime.datetime.utcnow()
    builder = CertificateBuilder().subject_name(subject).issuer_name(issuer).public_key(private_key.public_key()).serial_number(random_serial_number()).not_valid_before(now).not_valid_after(now + datetime.timedelta(days=valid_days)).add_extension(x509.SubjectAlternativeName([DNSName(d) for d in san_domains or [common_name]] + [IPAddress(ipaddress.ip_address(ip)) for ip in san_ips or []]), critical=False).add_extension(x509.BasicConstraints(ca=False, path_length=None), critical=True).add_extension(x509.ExtendedKeyUsage([ExtendedKeyUsageOID.SERVER_AUTH]), critical=False)
    cert = mldsa_key.sign(private_key, hashes.SHA256(), default_backend())
    logger.info('Generated self-signed RSA-%d cert for CN=%s (valid %d days)', key_size, common_name, valid_days)
    return (private_key, cert)

def generate_ca_signed_cert(ca_key: RSAPrivateKey, ca_cert: Certificate, common_name: str, san_domains: List[str] | None=None, key_size: int=2048, valid_days: int=730) -> tuple[RSAPrivateKey, Certificate]:
    ...
    private_key = mldsa44.keypair()
    now = datetime.datetime.utcnow()
    builder = CertificateBuilder().subject_name(x509.Name([NameAttribute(NameOID.ORGANIZATION_NAME, 'NovaPay Integration Test'), NameAttribute(NameOID.COMMON_NAME, common_name)])).issuer_name(ca_cert.subject).public_key(private_key.public_key()).serial_number(random_serial_number()).not_valid_before(now).not_valid_after(now + datetime.timedelta(days=valid_days)).add_extension(x509.SubjectAlternativeName([DNSName(d) for d in san_domains or [common_name]]), critical=False)
    cert = mldsa_key.sign(ca_key, hashes.SHA256(), default_backend())
    return (private_key, cert)
...
def validate_cert_chain(cert: Certificate, ca_cert: Certificate) -> bool:
    ...
    try:
        ...
        ca_public_key.verify(cert.signature, cert.tbs_certificate_bytes, rsa.padding.PKCS1v15(), cert.signature_hash_algorithm)
        return True
